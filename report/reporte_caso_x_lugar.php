...
$consulta=mysqli_query($con,"SELECT * FROM casos_juridicos CJ INNER JOIN estado ES ON ES.id_estado=CJ.id_estado INNER JOIN tipo_caso TC ON CJ.id_tipo_caso=TC.id_tipo_caso WHERE CJ.provincia='$provincias' and CJ.ciudad='$ciudades' and CJ.fecha BETWEEN '$inicio' and '$fin' ORDER BY id_casos_juridicos DESC");
$numCasos=mysqli_num_rows($consulta);

$consProvi=mysqli_query($con,"SELECT * FROM provincias WHERE provincia_id='$provincias'");
$rowProvi=mysqli_fetch_array($consProvi);
$consCiudad=mysqli_query($con,"SELECT * FROM ciudades WHERE ciudad_id='$ciudades'");
$rowCiudad=mysqli_fetch_array($consCiudad);
...
$y=$pdf->GetY();
$pdf->SetY($y);
$pdf->Cell(190,10,'REPORTE DE CASOS POR CIUDAD',0,1,'C');#(ancho,alto,texto,borde,salto linea,alineacion L C R)
...
$pdf->Cell(140,6,utf8_decode('Provincia: '.$rowProvi['provincia_nombre']),0,0,'L');
$pdf->Cell(70,6,utf8_decode('Desde: '.$inicio),0,1,'L');

$pdf->Cell(140,6,utf8_decode('Ciudad: '.$rowCiudad['ciudad_nombre']),0,0,'L');
$pdf->Cell(70,6,utf8_decode('Hasta: '.$fin),0,1,'L');
