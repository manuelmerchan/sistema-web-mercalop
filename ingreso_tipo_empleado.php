...
<div class="container delimitador">
  <div class="contenedor">
                ...
                <!-- <form class="form3" action="app/guardarTipoEmpleado.php" method="POST" enctype="multipart/form-data"> -->
                <form name="formulario1" class="form3" action="app/guardarTipoEmpleado.php" method="POST">
                    ...
                    <div class="row">
                      <div class="col-md-12 content_cajas">
                          ...
                          <input class="form_input" type="text" id="tipo_emple" name="tipo_emple" id="tipo_emple" onkeypress="return sololetras(event)" placeholder="Escriba Tipo De Empleado" onpaste="return false;" required>
                      </div>
...
function buscar_tip_emple(consulta){
  ...
  .done(function(respuesta){
    if(respuesta==''){
      ...
      if(respuesta>0){
        document.getElementById('tipo_emple').value='';
...
$(document).on('click','#guardar', function(){
  // var valr= $(this).val();
  var valr= document.getElementById('tipo_emple').value;
  if(valr!=""){
      ...
      title: 'COMPLETE TODOS LOS CAMPOS!!'
            ...
            <div class="cont_tabla">
              <table class="tabla">
                ...
                <tr>
                  <?php
                    $consulta=mysqli_query($con,"SELECT * FROM tipo_empleado ORDER BY id_tipo_empleado DESC");
                    while($row=mysqli_fetch_array($consulta)){
                  ?>
                  <td><?php echo $row['descrip_te']; ?> </td>
                  <td>
                    <div class="cont_tbn_tb">
                      <a href="editar_tipo_empleado.php?id=<?php echo $row['id_tipo_empleado']; ?>">
                        <button type="button" title="Modificar" class="btn btn-primary modificar" name="button">
                          <i class="far fa-edit fa-2x"> </i>
                        </button>
                      </a>
                      <button type="button" class="btn btn-danger eliminar" title="Eliminar" id="<?php echo $row['id_tipo_empleado'] ?>" name="button" id="elim">
                        <i class="far fa-trash-alt fa-2x" id="<?php echo $row['id_tipo_empleado'] ?>"> </i>
                      </button>
                    </div>
                    <!-- <?php if($row['id_tipo_empleado']!='13' && $row['id_tipo_empleado']!='1' && $row['id_tipo_empleado']!='3'){ ?>
                    <div class="cont_tbn_tb">
                      <a href="editar_tipo_empleado.php?id=<?php echo $row['id_tipo_empleado']; ?>">
                        <button type="button" title="Modificar" class="btn btn-primary modificar" name="button">
                          <i class="far fa-edit fa-2x"> </i>
  ...
  <script type="text/javascript">
    $('.eliminar').click(function(e){
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.value) {
          document.location.href="app/eliminarTipoEmpleado.php?id="+id_emp;
        } else if (
