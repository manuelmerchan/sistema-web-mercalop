...
<div class="container delimitador">
  <div class="contenedor">
                ...
                <!-- <form class="form3" action="app/guardarLugares.php" method="POST" enctype="multipart/form-data"> -->
                <form name="formulario1" class="form3" action="app/guardarLugares.php" method="POST">
                    ...
                    <div class="row">
                      <div class="col-md-12 content_cajas">
                          ...
                          <input class="form_input" type="text" id="lugares" name="lugares" onkeypress="return sololetras(event)" required placeholder="Escriba lugares" onpaste="return false;">
                      </div>
...
$(document).on('click','#guardar', function(){
  // var valr= $(this).val();
  var valr= document.getElementById('lugares').value;
  if(valr!=""){
    buscar_lugares(valr);
  }else{
    const Toast = Swal.mixin({
        ...
        <script>
            ...
            <div class="cont_tabla">
              <table class="tabla">
                ...
                <tr>
                  <?php
                    $consulta=mysqli_query($con,"SELECT * FROM lugares ORDER BY id_lugares DESC");
                    while($row=mysqli_fetch_array($consulta)){
                  ?>
                  <td><?php echo $row['descrip_l']; ?> </td>
                  <td>
                    <div class="cont_tbn_tb">
                      <a href="editar_lugares.php?id=<?php echo $row['id_lugares']; ?>">
                        <button type="button" title="Modificar" class="btn btn-primary modificar" name="button">
                          <i class="far fa-edit fa-2x"> </i>
                        </button>
                      </a>
                      <button type="button" class="btn btn-danger eliminar" title="Eliminar" id="<?php echo $row['id_lugares'] ?>" name="button" id="elim">
                        <i class="far fa-trash-alt fa-2x" id="<?php echo $row['id_lugares'] ?>"> </i>
                      </button>
                    </div>
                  </td>
                </tr>
  <script type="text/javascript">
    $('.eliminar').click(function(e){
      ...
      swalWithBootstrapButtons.fire({
        ...
        if (result.value) {
          document.location.href="app/eliminarLugares.php?id="+id_emp;
        } else if (
